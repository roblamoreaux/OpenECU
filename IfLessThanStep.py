...
@attribute(OpenTap.Display("If Less Than", "Compare two values and run child if true.", "Flow Control"))
#AllowAnyChildAttribute is attribute that allows any child step to attached to this step
@attribute(OpenTap.AllowAnyChild())
class IfLessThanStep(TestStep):
    __clr_attribute__ = [Display("If Value1 Less Than Value2 Step")]
...
#            self.Results = 1;
            self.log.Debug("{0} is {1} Than {2}", self.Value1, "Less", self.Value2)
